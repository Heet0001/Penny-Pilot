...
function displayCombinedDebtList(debts, transactions) {
    ...
    debts.forEach(debt => {
        allItems.push({
            type: 'debt',
            data: debt,
            // Use created_at if available, otherwise due_date
    ...
    allItems.forEach(item => {
        ...
        if (item.type === 'debt') {
            ...
            icon = debt.debt_type === "given" ? "fa-money-bill-wave" : "fa-hand-holding-usd";
            amountClass = debt.debt_type === "given" ? "debt-out" : "debt-in";
            amountSign = debt.debt_type === "given" ? "-" : "+";
            title = `${debt.debt_type === "given" ? "You owe" : "Owes you"}: ${debt.counterparty || "Unknown"}`;
...
function checkOverdueDebts(debts) {
    const overdueDebts = debts.filter((debt) => debt.is_overdue);
    const upcomingDebts = debts.filter(
        (debt) =>
            !debt.is_overdue &&
            debt.days_remaining <= 3 &&
    ...
    if (overdueDebts.length > 0 || upcomingDebts.length > 0) {
        ...
        overdueDebts.forEach((debt) => {
            ...
            notification.innerHTML = `
                ...
                <p style="margin: 5px 0;">
                    ${debt.debt_type === "given" ? "Debt given to" : "Debt received from"} 
                    ${debt.counterparty || "Unknown"} is overdue by 
                    ${Math.abs(debt.days_remaining)} days.
                </p>
        ...
        upcomingDebts.forEach((debt) => {
            ...
            notification.innerHTML = `
                ...
                <p style="margin: 5px 0;">
                    ${debt.debt_type === "given" ? "Debt given to" : "Debt received from"} 
                    ${debt.counterparty || "Unknown"} is due in 
                    ${debt.days_remaining} days.
                </p>
                <p style="margin: 5px 0;">Amount: $${parseFloat(debt.remaining_amount).toFixed(2)}</p>
            `;
...
document.getElementById("save-collection")?.addEventListener("click", function() {
    ...
    // Get selected debt
    const selectedDebt = window.loadedDebts.find(
        (debt) => debt.id == debtSelect.value
    );
    if (!selectedDebt) {
    ...
    if (amount > parseFloat(selectedDebt.remaining_amount)) {
        alert("Amount cannot exceed the remaining debt amount!");
        return;
    }
    ...
    fetch("http://localhost:3000/collect-debt", {
        ...
        .then((data) => {
            console.log("Debt Collection Processed Successfully:", data);
            alert(
                `Debt ${
                    selectedDebt.debt_type === "given" ? "collection" : "payment"
                } processed successfully!`
            );

            // Update wallet balance locally
            updateWalletBalanceLocal(
                amount,
                selectedDebt.debt_type === "given" // If debt was given, collecting adds to wallet
            );
        ...
        .catch((error) => {
            console.error("Error processing debt collection:", error);
            alert("Failed to process debt collection. Please try again.");
        });
});
...
document.addEventListener("DOMContentLoaded", function() {
    ...
    if (pdfExportForm) {
        pdfExportForm.addEventListener('submit', function(e) {
            e.preventDefault();
            const reportType = document.querySelector('input[name="reportType"]:checked').value;
            let fromDate, toDate;
...
function populateDebtSelect() {
    ...
    // Filter to only show active debts (not fully paid)
    ...
    // Add active debts to the dropdown
    activeDebts.forEach(debt => {
        const option = document.createElement("option");
        option.value = debt.id;
        option.textContent = `${debt.debt_type === "given" ? "You → " : "← "}${debt.counterparty || "Unknown"} ($${parseFloat(debt.remaining_amount || debt.amount).toFixed(2)})`;
...
function updateDebtDetails(debt) {
    ...
    document.getElementById("interest-amount").textContent = `$${parseFloat(debt.current_interest || 0).toFixed(2)}`;
    document.getElementById("remaining-amount").textContent = `$${parseFloat(debt.remaining_amount || debt.amount).toFixed(2)}`;
    ...
    const amountInput = document.getElementById("collection-amount");
    if (amountInput) {
        amountInput.max = parseFloat(debt.remaining_amount || debt.amount).toFixed(2);
        amountInput.placeholder = `Max: $${parseFloat(debt.remaining_amount || debt.amount).toFixed(2)}`;
    }
}
