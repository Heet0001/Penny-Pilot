...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Load client-specific payment reminders
    loadClientReminders();
...
function setupEventListeners() {
    ...
    // Client reminder form submission
    const reminderForm = document.getElementById('clientReminderForm');
    if (reminderForm) {
        reminderForm.addEventListener('submit', handleReminderSubmit);
    }

    // Reminder notification toggle
    const reminderToggle = document.getElementById('client-reminder-toggle');
    const reminderPanel = document.getElementById('client-reminder-container');
    if (reminderToggle && reminderPanel) {
        reminderToggle.addEventListener('click', function(event) {
        ...
        // Hide reminder panel when clicking outside
...
function displayClientsGrid() {
    ...
    container.innerHTML = allClients.map(client => {
        ...
        const reminderIcon = clientReminders && clientReminders[client.id] && clientReminders[client.id].active !== false
            ? '<span class="client-reminder-icon" title="Monthly payment reminder set"><i class="fas fa-bell"></i></span>'
            : '';
            ...
            <div class="client-card fade-in" data-client-id="${client.id}">
                <div class="client-header">
                    <div>
                        ...
                        <div class="client-type">${client.customer_type} ${reminderIcon}</div>
                    </div>
                ...
                <div class="client-revenue">
                    <span class="client-revenue-amount">₹${(client.total_revenue || 0).toLocaleString()}</span>
                    <span class="client-revenue-meta">Income ₹${(client.revenue_collected || 0).toLocaleString()} · Expenses ₹${(client.total_expenses || 0).toLocaleString()}</span>
                </div>
                ...
                <div class="client-actions">
                    ...
                    <button class="btn btn-secondary" onclick="openReminderModal(${client.id})">
...
function displayClientsTable() {
    ...
    tbody.innerHTML = allClients.map(client => {
        ...
        return `
            <tr>
                <td>${clientName}</td>
                <td><span class="client-type">${client.customer_type}</span> ${reminderIcon}</td>
                <td>${client.email}</td>
                <td>${client.phone}</td>
                ...
                <td>
                    ...
                    <button class="btn btn-secondary" onclick="openReminderModal(${client.id})" title="Payment Reminder">
...
function openTransferWalletModal() {
    // Calculate net revenue (income - expenses across all clients)
    ...
    // Subtract what was already transferred to get the real available amount
    const transferredData = localStorage.getItem('clientsTransferredToWallet');
    ...
    const availableToTransfer = Math.max(0, netRevenue - alreadyTransferred);

    // Update modal title and subtitle with the real available amount
    ...
    if (titleEl) {
        titleEl.innerHTML = `
            ...
            <div style="font-size: 14px; color: #6B7280; font-weight: normal; margin-top: 5px;">
                Available to Transfer: ₹${availableToTransfer.toLocaleString('en-IN')}
            </div>
        `;
    }

    // Set maximum amount to what's actually available
    const amountInput = document.getElementById('transfer-wallet-amount');
    amountInput.value = '';
    amountInput.max = availableToTransfer;
    amountInput.placeholder = `Max: ₹${availableToTransfer.toLocaleString('en-IN')}`;
...
async function handleTransferWalletSubmit(event) {
    ...
    if (amount > moneyLeft) {
        showAlert(`Insufficient money left. Available for transfer: ₹${moneyLeft.toLocaleString('en-IN')}`, 'error');
        return;
    }
    ...
    try {
        ...
        // /update-wallet (doing both would double-credit the wallet).
        const creditData = {
        ...
        if (!creditResponse.ok) {
            ...
            throw new Error(errBody.error || `Server rejected the transfer (HTTP ${creditResponse.status})`);
        }

        // Mirror the new balance locally so any cached UI is in sync.
        ...
        console.log('Wallet updated via /add-credit:', {
        ...
        // Store as structured data with userId so statistics can read it correctly
        const transferredPayload = {
            userId: currentUser.id,
            totalTransferred: newTotalTransferred
        };
        localStorage.setItem('clientsTransferredToWallet', JSON.stringify(transferredPayload));
        ...
        hideLoading();
        showToastSuccess(`Successfully transferred ₹${amount.toLocaleString('en-IN')} to main wallet!`);
...
// Client Payment Reminder Utilities
function loadClientReminders() {
    if (!currentUser) {
        clientReminders = {};
        return;
    }

    const key = `clientPaymentReminders_${currentUser.id}`;
    ...
    try {
        ...
        clientReminders = parsed && typeof parsed === 'object' ? parsed : {};
    } catch (error) {
        console.warn('Error parsing client reminders, resetting:', error);
        clientReminders = {};
    }
}
...
function saveClientReminders() {
    ...
    const key = `clientPaymentReminders_${currentUser.id}`;
    localStorage.setItem(key, JSON.stringify(clientReminders || {}));
}

function getMonthlyReminderContext() {
    const today = new Date();
    const monthKey = `${today.getFullYear()}-${String(today.getMonth() + 1).padStart(2, '0')}`;
    return { today, monthKey };
}

function getDueClientReminders() {
    if (!clientReminders || !allClients || allClients.length === 0) return [];

    const { today, monthKey } = getMonthlyReminderContext();

    return Object.entries(clientReminders)
        .map(([id, config]) => {
        ...
        .filter(item => {
            const { client, config } = item;
            if (!client || !config || config.active === false) return false;

            const day = parseInt(config.dayOfMonth, 10);
            if (isNaN(day) || day < 1 || day > 31) return false;

            const dueDate = new Date(today.getFullYear(), today.getMonth(), day);
            if (today < dueDate) return false;

            if (config.lastNotified === monthKey) return false;
...
function updateReminderNotificationUI() {
    const dueReminders = getDueClientReminders();
    const badge = document.getElementById('client-reminder-count');
    const container = document.getElementById('client-reminder-container');
    ...
    if (dueReminders.length === 0) {
        container.innerHTML = `
            <div class="reminder-empty">
                ...
                <div>
                    <div class="reminder-empty-title">No pending payment reminders</div>
                    <div class="reminder-empty-subtitle">You're all caught up for this month.</div>
                </div>
            </div>
        `;
        return;
    ...
    container.innerHTML = dueReminders.map(item => {
        const { client, config, clientId } = item;
        const clientName = client.customer_type === 'business'
            ? client.company_name
            : `${client.first_name} ${client.last_name}`;
        ...
        return `
            <div class="reminder-item">
                <div class="reminder-item-header">
                    <div>
                        <div class="reminder-item-title">${clientName}</div>
                        <div class="reminder-item-subtitle">
                            Monthly on ${config.dayOfMonth} • Amount: ₹${amount.toLocaleString('en-IN')}
                        </div>
                    </div>
                    <button class="reminder-mark-btn" onclick="acknowledgeClientReminder(${clientId})">
                        Mark done
                    </button>
                </div>
                ${config.note ? `<div class="reminder-item-note">${config.note}</div>` : ''}
            </div>
        `;
    }).join('');
}
...
function openReminderModal(clientId) {
    ...
    if (!client) {
        showToastError('Client not found for reminder.');
        return;
    }
    ...
    const titleEl = document.getElementById('reminder-client-name');
    if (titleEl) {
        titleEl.textContent = clientName;
    }

    const config = clientReminders && clientReminders[clientId] ? clientReminders[clientId] : {};

    const dayInput = document.getElementById('reminder-day');
    const amountInput = document.getElementById('reminder-amount');
    const noteInput = document.getElementById('reminder-note');
    const activeInput = document.getElementById('reminder-active');

    if (dayInput) dayInput.value = config.dayOfMonth || 1;
    if (amountInput) amountInput.value = config.amount != null ? config.amount : '';
    if (noteInput) noteInput.value = config.note || '';
    if (activeInput) activeInput.checked = config.active !== false;

    const modal = document.getElementById('clientReminderModal');
    if (modal) {
        modal.style.display = 'block';
    }
}
...
function handleReminderSubmit(event) {
    ...
    if (!currentUser || currentClientId === null) {
        showToastError('Please select a client for the reminder.');
        return;
    }
    ...
    if (isNaN(day) || day < 1 || day > 31) {
        showToastError('Please enter a valid reminder day between 1 and 31.');
        return;
    }

    if (isNaN(amount) || amount <= 0) {
        showToastError('Please enter a valid reminder amount.');
        return;
    }
    ...
    const existing = clientReminders[currentClientId] || {};
    clientReminders[currentClientId] = {
        dayOfMonth: day,
        amount: amount,
        note: note,
        active: active,
    ...
    saveClientReminders();
    updateReminderNotificationUI();
    showToastSuccess('Monthly payment reminder saved for this client.');
    closeReminderModal();
}

function acknowledgeClientReminder(clientId) {
    if (!clientReminders || !clientReminders[clientId]) return;

    const { monthKey } = getMonthlyReminderContext();
    clientReminders[clientId].lastNotified = monthKey;
    saveClientReminders();
    updateReminderNotificationUI();
    showToastSuccess('Marked reminder as done for this month.');
}
...
window.acknowledgeClientReminder = acknowledgeClientReminder;
