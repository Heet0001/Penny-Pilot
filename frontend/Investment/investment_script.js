...
function renderPortfolioChart(investments) {
  ...
  if (pieChartPlaceholder) {
    ...
    // Canvas dimensions are governed by the shared chart sizing CSS variables
    const canvas = document.createElement('canvas');
    canvas.id = 'portfolioChart';
    canvas.style.width = '100%';
    canvas.style.maxHeight = 'var(--chart-canvas-height, 300px)';
  ...
  if (barChartPlaceholder) {
    ...
    barCanvas.style.maxHeight = 'var(--chart-canvas-height, 300px)';
      ...
      try {
        ...
        if (data.price) {
          ...
          if (data.source === 'alphavantage') {
            ...
            const resolved = data.resolved_symbol && data.resolved_symbol !== data.symbol
              ? ` [${data.resolved_symbol}]`
              : '';
            showToastSuccess(`Live price for ${stockName}${resolved}: ₹${price.toFixed(2)}${cachedTag}`);
          } else if (data.source === 'mock') {
            const reasonMap = {
              rate_limited: 'Alpha Vantage rate limit reached (free tier = 25/day). Showing mock price - try again later.',
              no_key: 'Alpha Vantage API key not configured on the server.',
              invalid_symbol: `"${stockName}" was not recognised by Alpha Vantage. Check the symbol or enter price manually.`,
              fetch_error: 'Could not reach Alpha Vantage. Showing mock price.',
              empty_response: 'Alpha Vantage returned no data for this symbol. Showing mock price.',
            };
            const msg = reasonMap[data.reason] || `Showing mock price ₹${price.toFixed(2)} for ${stockName}.`;
            showToastInfo(msg);
          }
        } else {
