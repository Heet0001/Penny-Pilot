...
// In Functionality.js, around line 649
function initializeEmergencyFund() {
    ...
    fetch(`http://localhost:3000/get-emergency-fund/${currentUser.id}`)
        ...
        .then(data => {
            console.log('Emergency fund response:', data); // Debug log
            if (data && typeof data.balance !== 'undefined') {
                // Handle both number and string balances
                const balance = typeof data.balance === 'number' ? 
                    data.balance : 
                    parseFloat(data.balance);
                    
                localStorage.setItem(
                    `emergency_fund_${currentUser.id}`, 
                    balance.toFixed(2)
                );
                displayEmergencyFundBalance();
            }
        ...
        .catch(error => {
            console.error('Error fetching emergency fund data:', {
