...
// Update the /get-entries endpoint to handle date filtering
app.get("/get-entries", (req, res) => {
  ...
  let creditWhere = "user_id = ?"
  let debitWhere = "user_id = ?"
  const creditParams = [user_id]
  const debitParams = [user_id]

  if (from_date) {
    creditWhere += " AND entry_date >= ?"
    debitWhere += " AND entry_date >= ?"
    creditParams.push(from_date)
    debitParams.push(from_date)
  }

  if (to_date) {
    creditWhere += " AND entry_date <= ?"
    debitWhere += " AND entry_date >= ?"
    creditParams.push(to_date)
    debitParams.push(to_date)
  }

  // Get both credit and debit entries with date filtering
  const creditSql = `SELECT * FROM credit_entries WHERE ${creditWhere} ORDER BY entry_date DESC`
  const debitSql = `SELECT * FROM debit_entries WHERE ${debitWhere} ORDER BY entry_date DESC`

  db.query(creditSql, creditParams, (creditErr, creditResults) => {
    if (creditErr) {
      console.error("Database error fetching credit entries:", creditErr)
      return res.status(500).json({ error: "Database error occurred" })
    }

    db.query(debitSql, debitParams, (debitErr, debitResults) => {
      if (debitErr) {
...
// Get expenses grouped by category (fixed)
app.get("/get-expenses-by-category/:userId", (req, res) => {
  const userId = req.params.userId
...
// Get income grouped by category (fixed)
app.get("/get-income-by-category/:userId", (req, res) => {
  const userId = req.params.userId
